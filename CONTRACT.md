...
PR URLs are normalized HTTPS `github.com/{owner}/{repo}/pull/{number}` URLs only. Manual tasks accept an optional PR gate; standalone PR tasks require one and containers reject one. Removing a gate is allowed only for manual tasks. Omitted update fields remain unchanged; changing a PR URL switches to that URL's workspace-local verification without changing `manualDone`. Polling verifies attached manual gates like standalone PR tasks and must not apply responses for URLs no longer tracked during a request.

`prMergeStatus` is informational GitHub gate readiness, separate from task `status`; it never satisfies an own condition. Only REST-verified `prState: "merged"` satisfies a PR gate. Verification is cached once per normalized PR URL within each workspace, so every task referencing that URL observes the same state in O(1) lookup time. New references and imports immediately reuse cached verification; unseen URLs default to `unknown`. The cache persists locally across detachment and deletion but is excluded from portable state. Task mutation APIs do not accept verification fields.

Polling runs only while the server runs. Each polling generation fetches every tracked normalized URL at most once with bounded concurrency, then atomically commits all still-current URL results. Failures retain last-verified values and report sanitized errors, so completion can reflect stale verification. A non-null `prError` means stale/incomplete verification; `prCheckedAt` is the latest attempt, not necessarily a successful check. A successful GitHub status/refresh HTTP response is not proof that GitHub succeeded: inspect status/error fields. See [PR verification](docs/pr-verification.md) for readiness precedence and partial-failure rules.

## External Links

Every task kind, including containers, may carry up to five ordered external links. Each link has an HTTPS URL without embedded credentials, a label string that may be empty, and a user-editable type of `github`, `jira`, `google-doc`, or `generic`. URLs are normalized and must be unique within the task. External links are contextual only: they are never fetched by the server and never affect completion or PR verification. UI inference supplies an editable default type from recognized URLs; the saved type is authoritative and may intentionally differ from the URL.

## Task Markdown Context

Each task may have one optional Markdown README, separate from its description, `TaskView`, and portable manifest record. The server-managed `GET`, `PUT`, and `DELETE /tasks/:id/readme` routes require an existing task and obey normal workspace scoping. Missing content reads as an empty string; saving empty content or deleting removes the sidecar. Content never affects completion. JSON requests remain subject to the API's 256kb body limit, while stored and synced README content is capped at 1 MB.

README sidecars are workspace-local files beside the workspace database and must not be edited directly by clients. Deleting a task removes its README; cascading deletion removes README files for all owned descendants in the deletion set. Workspace removal deletes its README directory with its other local data.
...
Favorites is the permanent system tag `favorites` (`Favorites`, `#d4af37`). It is synthesized on every stored-state read and cannot be created, edited, or deleted. Favorites membership does not count toward the custom-tag limit. Whole custom-set replacement preserves Favorites; individual membership operations are idempotent. Task create/update may include an absolute `favorite` boolean so fields, custom tags, and Favorites commit atomically. Deleting a custom tag requires confirmation and atomically strips it from every task.

## Workspaces

Workspace management is always unscoped. Domain requests may use the persisted default or an explicit workspace; **unknown or removed explicit IDs fail without fallback**. Unscoped domain requests return 404 when no default exists, while management and health remain accessible. Workspaces isolate local graphs, layouts, UI preferences, and sync bookkeeping. UI preferences persist locally and are excluded from portable cloud state.
...
Cloud workspaces remain local-first SQLite graphs, not hosted databases. Create/connect save configuration without remote reads or writes; success does not prove access or import/publish state. A confirmed cloud retarget preserves local data, invalidates old previews, and makes no remote request; it rejects an active sync or uncertain old upload. Users must review and apply a new sync preview after configuration changes before importing from or publishing to the selected target. Cloud-to-local demotion is not supported. Cloud defaults are branch `main`, path `foggybrain/state.json`, and credential `dedicated`. Sync requires a private repository and existing branch.
...
Workspace removal instead requires explicit confirmation and a current preview revision fingerprint. Preview is local-only and includes counts, dirty status, and blocking reasons. Stale previews and active state sync block removal. The final workspace may be removed. Removal deletes local registration, graph, layouts, UI preferences, sync bookkeeping, unsynced changes, and backups, never GitHub files. Local runtime services stop before database deletion; failed file cleanup is retained durably and retried on startup. Run only one server per data directory; multiple active in-process managers block removal.
...
Portable versioned JSON preserves editable task fields including external links, tag and relationship identities, but excludes layouts, timestamps, derived completion, and PR verification. Version 1 input upgrades in memory with no custom tags, memberships, or external links; version 2 upgrades with no external links; output is always version 3. The canonical Favorites definition is omitted and rejected in portable files, while `favorites` memberships remain valid. Runtime validation checks imported fields, tag memberships, and the whole merged graph using the existing completion/cycle algorithm. Imported PRs reuse workspace-local verification by normalized URL. Layouts are preserved and cleaned of removed tasks.

Each task may have an optional Markdown README stored locally outside SQLite. For a state target `work/work.json`, sync stores its README at `work/tasks/{task-id}.md`; the README path is derived from the task ID and never stored in the manifest. README changes participate in preview change lists (`collection: "readmes"`), conflicts (`readmes/{task-id}`), resolution, revert, and dirty detection independently from graph state. Apply publishes the state manifest and managed README sidecars in one Git commit; unmanaged repository files are never touched.
...
**Revert to origin** is exact local portable graph and README sidecar replacement from the configured target, not a merge, `resolve remote`, or GitHub rollback. Preview freshly fetches an existing valid remote file and its managed sidecars; valid empty remote state may delete all local tasks and context, but missing/invalid manifests never imply clearing state. Revert rejects conflict resolution and reports local changes with no remote changes/conflicts. Uncertain pending uploads block revert until reconciled.

Revert apply uses the server-saved preview mode, refetches and checks the reviewed remote SHA/content, verifies local portable state, README sidecars, and sync bookkeeping, backs up the latest full SQLite graph snapshot, replaces the graph and README sidecars, and updates baseline/last-sync time. It never writes GitHub or creates upload intent. Layouts and matching verification follow ordinary import rules. Review and authorize all discarded local differences explicitly. Remote state can change after revalidation: replacement uses the reviewed and revalidated revision, not a remote lock.

Sync uses GitHub's Contents API for state reads and Git Database API commits for atomic manifest/README sidecar writes, never a local git CLI or clone. Manifest output uses two-space-indented JSON with a trailing newline. Full SQLite graph snapshots are backed up in `foggybrain_sync_backups` before applying sync, with no restore API or pruning; README sidecars are not part of those backup rows. Git history and backups retain deleted sensitive text; deletion is not secure erasure.
