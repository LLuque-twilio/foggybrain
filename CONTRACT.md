...
Cloud workspaces remain local-first SQLite graphs, not hosted databases. Create/connect save configuration without remote reads or writes; success does not prove access or import/publish state. A confirmed cloud retarget preserves local data, invalidates old previews, and makes no remote request; it rejects an active sync or uncertain old upload, and users must review/apply a new sync preview before either target changes. Cloud-to-local demotion is not supported. Cloud defaults are branch `main`, path `foggybrain/state.json`, and credential `dedicated`. Sync requires a private repository and existing branch.
...
Sync uses GitHub's Contents API for state reads and Git Database API commits for atomic manifest/README sidecar writes, never a local git CLI or clone. Manifest output uses two-space-indented JSON with a trailing newline. Full local snapshots are backed up in `foggybrain_sync_backups` before applying sync, with no restore API or pruning. Git history and backups retain deleted sensitive text; deletion is not secure erasure.
