...
Sync uses GitHub's Contents API, not a local git CLI/clone; writes use the reviewed file SHA and publish two-space-indented JSON with a trailing newline. Full local snapshots are backed up in `foggybrain_sync_backups` before applying sync, with no restore API or pruning. Git history and backups retain deleted sensitive text; deletion is not secure erasure.
