...
For standalone installation, use `pnpm install --frozen-lockfile`, `pnpm pack` (its `prepack` hook runs `pnpm build`), then `npm install --global ./foggybrain-0.1.0.tgz` into a user-writable prefix on `PATH`, without sudo. Alternatively use `pnpm add -g ./foggybrain-0.1.0.tgz` after `pnpm setup` and reopening the terminal. No publish or checkout link is required; keep `private: true`. The package allowlist includes bin, dist, README, LICENSE, CONTRACT, openapi, and docs, excluding compiled test JS. Inspect actual tarball contents when changing packaging. Verify in isolated temporary storage; do not globally install on the user's machine without authorization.
...
Use source execution with quiet pnpm output, or the tarball-installed `foggy` (optional `pnpm link` after building is for checkout-linked development):
