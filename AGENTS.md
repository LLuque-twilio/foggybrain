...
Sync configuration and credentials belong only on the server: `FOGGY_SYNC_REPO=owner/repo`, an existing `FOGGY_SYNC_BRANCH` (default `main`), `FOGGY_SYNC_PATH` (default `foggybrain/state.json`), and explicit dedicated `FOGGY_SYNC_TOKEN`. Restrict Contents read/write to only the selected PRIVATE state repository; no `GH_TOKEN` fallback. Never perform real GitHub writes for tests. Sync uses the Contents API, not a local git CLI/clone. Portable versioned JSON excludes verification, derived completion, layouts, and timestamps; PRs are reverified locally. SQLite stores the baseline and automatic full backups in `foggybrain_sync_backups`, with no restore API or pruning. Git history and backups retain deleted sensitive text.
