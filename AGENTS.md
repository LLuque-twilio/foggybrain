...
Each task may have optional Markdown context through `GET`, `PUT`, and `DELETE /api/tasks/:id/readme`, with the usual explicit-workspace route prefix when selected. The CLI has no README command. Read existing content before replacement, preserve the same workspace selection, and never edit server-managed sidecar files directly. Empty content removes the sidecar. Task deletion also removes context for every task in its owned deletion set.
...
Management is always unscoped: `workspace list`, `workspace create NAME [--type local|cloud]`, `workspace rename ID NAME`, `workspace connect ID --repo OWNER/REPO`, and `workspace retarget ID --repo OWNER/REPO --yes`. Cloud create/connect/retarget require a private repo, existing branch, and available selected credential, defaulting omitted values to `--branch main --path foggybrain/state.json --credential dedicated`. Connect only converts local to cloud. Retarget requires authorization, preserves local data, invalidates old previews, makes no remote request, and rejects active sync or uncertain upload; pass unchanged target values explicitly because omitted CLI options reset to defaults. Cloud demotion is unsupported. Existing databases initially remain workspace `default`; new IDs are server-generated. Configuration changes never import or publish state. Review/apply a new scoped sync preview after create, connect, or retarget; do not confuse configuration success with verified access.
...
`foggy --json --workspace ID sync preview --revert` previews **Revert to origin**: exact local graph and task-context replacement from the configured GitHub branch/path, never a GitHub write. Inspect `mode`, target, every `localChanges` entry including `collection: "readmes"`, and `canApply`; resolve README IDs to task titles when reporting impact. Obtain authorization to discard those differences, then use the same workspace for `sync apply REVIEWED_PREVIEW_ID --yes`. Do not confuse this with `--resolve remote`, which merges nonconflicting local work. Missing/invalid remote files and uncertain pending uploads block revert; valid empty remote state can delete all local tasks and context. Apply revalidates the remote revision and local state, creates a full SQLite graph backup, and updates the baseline atomically. Never retry a timed-out revert blindly; inspect state and re-preview.

Manual state sync is the top-level `sync` group, not `github sync` PR polling. Use `foggy --json sync status` then `foggy --json sync preview`; inspect both change lists, `conflicts`, `validationError`, and `canApply`, even on exit `0`. Change lists can include `collection: "readmes"`; README conflicts use `readmes/TASK_ID` and contain complete Markdown values. Obtain authorization before `foggy --json sync apply REVIEWED_PREVIEW_ID --yes`. Apply never prompts or creates a preview. `--resolve local|remote` chooses that side for every conflict in a new preview, including README conflicts; it is not a force override. Preview IDs are process-local, single-use, and replaced by newer previews. Re-preview after restart, edits, failures, or timeouts; never retry a timed-out apply blindly because a remote write may have committed. Uncertain outcomes may need reconciliation. With no shared baseline and two nonempty sides, preserve both; do not wipe data to bypass the guard.

Sync credentials belong only on the server. Dedicated mode (the safe default) uses `FOGGY_SYNC_TOKEN` without fallback. Only explicit `--credential github` opts into server `GH_TOKEN`, `GITHUB_TOKEN`, or `gh auth token` reuse. Either mode needs Contents read/write on the selected PRIVATE state repository and any organization/SSO approval; prefer dedicated credentials to keep PR access read-only. Legacy `FOGGY_SYNC_REPO`, `FOGGY_SYNC_BRANCH`, and `FOGGY_SYNC_PATH` configure the default target; additional workspace targets use management commands. Never perform real GitHub writes for tests. Sync uses the Contents API for reads and Git Database API for atomic manifest/README commits, not a local git CLI/clone. Portable versioned JSON excludes task README content, verification, derived completion, layouts, and timestamps; README sidecars sync separately and PRs are reverified locally. SQLite stores the baseline and automatic full graph backups in `foggybrain_sync_backups`, with no restore API or pruning; these backups do not include README sidecars. Git history and backups retain deleted sensitive text.

`GH_TOKEN` belongs in the server environment, `~/.foggybrain/config.json` (mode `0600`, written by `foggy config`), or an ignored `.env`, never source, task text, command URLs, logs, or committed fixtures. Fine-grained token access should be read-only and limited to relevant repositories: Metadata and Pull requests read, Contents read if required, plus any organization/SSO approval. Do not claim universal repository access. GitHub polling runs only while the server runs; poll errors can leave stale last-verified PR state. Report stale/error conditions rather than inventing successful syncs.
