...
foggy workspace retarget <id> --repo owner/repo --yes [--branch branch] [--path path] [--credential dedicated|github]
```
...
`foggy upgrade` resolves the latest GitHub release (or `--version`) through GitHub's `releases/latest` endpoint, which **excludes prereleases**: upgrading to one requires `--version`, and on a repository whose only release is a prerelease, a bare `foggy upgrade` fails with `HTTP 404`. The request is unauthenticated, so it also shares that IP's 60-requests-per-hour API budget; exhausting it fails with `HTTP 403` until the hour resets. Neither failure affects a pinned `--version` upgrade, which downloads release assets directly rather than through the API. It then downloads `SHA256SUMS` and `foggybrain-<version>.tar.gz` from that release, aborts unless the archive's SHA-256 matches its `SHA256SUMS` entry (the same check `scripts/install.sh` makes), extracts it to `~/.foggybrain/versions/<version>/`, repoints `~/.foggybrain/current` and `~/.local/bin/foggy`, and returns `{"version":"...","path":"...","bin":"...","pathEntry":"present","previousVersion":"..."}`. Old version directories are kept for rollback; remove them with `rm -rf ~/.foggybrain/versions/<old>`. A server started before the upgrade keeps running the old code out of its still-present version directory, so restart it with `foggy stop && foggy start`.
